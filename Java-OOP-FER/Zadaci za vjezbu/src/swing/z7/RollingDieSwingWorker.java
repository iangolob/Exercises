...
import swing.z6.RollingDieGUIResponsive;
...
public class RollingDieSwingWorker extends JFrame {
	...
	public RollingDieSwingWorker() {
		...
		startBtn.addActionListener((e) -> {
			...
			SwingWorker worker = new RollingTask(slider.getValue());
			worker.execute();
	...
	private class RollingTask extends SwingWorker<Integer, Integer> {
		private int sliderValue;
		private int current = 0;

		public RollingTask(int value) {
			sliderValue = value;
			numOfThrows=0;
		}
		...
		protected Integer doInBackground() throws Exception {
			while (current < sliderValue) {
				Thread.sleep(1);
				publish(((int) (Math.random() * 6)) + 1);
				current++;
				numOfThrows++;
			}
		...
		protected void process(List<Integer> chunks) {
			...
			for (Integer i : chunks)
				values[i - 1]++;
			for (int i = 0; i < 6; i++) {
				int lastValue = Integer.parseInt(numList.get(i).getText());
				numList.get(i).setText(Integer.toString(lastValue + values[i]));
			}
		}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			JFrame frame = new RollingDieSwingWorker();
			frame.pack();
